 ...
 * 本脚本用于测试全国资讯二级页面是否有数据重复，方法是通过请求多次全国资讯二级页面，获取每条资讯的news_id值，再判断这些news_id是否有重复
 */

public class NewsSecondPageRequestTest {
/*	
	//test
...
*/	
	...
	public static void main (String[] args) {
		...
		NewsSecondPageRequestTest newssecondpagetest = new NewsSecondPageRequestTest();
		
		int requesttimes = 10; //请求10次
		for (int i=0; i<requesttimes; i++) {
			String dataresult = newssecondpagetest.getNews();
			List<String> nationalnewslist = newssecondpagetest.getNationalNewsList(dataresult);
			System.out.println("news数量为: " + nationalnewslist.size());
			totalnationalnewslist.addAll(nationalnewslist);
		}
		
		System.out.println("news的总数量为: " + totalnationalnewslist.size());
